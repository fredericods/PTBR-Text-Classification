...
def preprocess_dataset(input_file='B2W-Reviews01.csv', output_file='b2w_preprocessed.csv', nfolds=5, preprocess_corpus=True, text_column='review_text'):
    ...
    try:
        data = pd.read_csv(os.path.join('0_data','datasets_b2w',input_file))
    except:
        data = pd.read_csv(os.path.join('0_data','datasets_b2w',input_file), sep=';')
    ...
    # Save csv
    data.to_csv(os.path.join('0_data','datasets_b2w',output_file), index=False)
...
def tokenize_corpus(input_file, text_column, output_file):
    ...
    with open(os.path.join('0_data','datasets_b2w',output_file + '_lemma' + '.csv'),'w', newline='') as f:
        wr = csv.writer(f)
        wr.writerows(tokenized_corpus_lemma)
    with open(os.path.join('0_data','datasets_b2w',output_file + '_stem' + '.csv'),'w', newline='') as f:
        wr = csv.writer(f)
        wr.writerows(tokenized_corpus_stem)
    with open(os.path.join('0_data','datasets_b2w',output_file + '_none' + '.csv'),'w', newline='') as f:
        wr = csv.writer(f)
